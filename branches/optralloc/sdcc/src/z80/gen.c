...
aopGetLitWordLong (const asmop * aop, int offset, bool with_hash)
{
  ...
  switch (aop->type)
    ...
    case AOP_IMMD:
      ...
      if (with_hash)
        {
          tsprintf (buffer, sizeof (buffer), "!hashedstr + %d", aop->aopu.aop_immd, offset);
        }
      else if (offset == 0)
        {
          tsprintf (buffer, sizeof (buffer), "%s", aop->aopu.aop_immd);
        }
      else
        {
          tsprintf (buffer, sizeof (buffer), "%s + %d", aop->aopu.aop_immd, offset);
        }
  ...
  switch (aop->type)
    ...
    case AOP_STK:
      if (IS_GB)
      ...
      else
        {
          ...
          tsprintf (buffer, sizeof (buffer), "!*ixx", aop->aopu.aop_stk + offset);
        }
    ...
    case AOP_PAIRPTR:
      ...
      if (aop->aopu.aop_pairId == PAIR_IX)
        ...
        SNPRINTF (buffer, sizeof (buffer), "(%s)", _pairs[aop->aopu.aop_pairId].name);
      ...
      if (isLitWord (AOP (IC_LEFT (ic))))
      ...
      else
        {
          ...
          if (AOP_TYPE (IC_LEFT (ic)) != AOP_REG ||
              AOP (IC_LEFT (ic))->aopu.aop_reg[0]->rIdx != B_IDX && AOP (IC_LEFT (ic))->aopu.aop_reg[0]->rIdx != C_IDX
              && AOP (IC_LEFT (ic))->aopu.aop_reg[1]->rIdx != B_IDX && AOP (IC_LEFT (ic))->aopu.aop_reg[1]->rIdx != C_IDX)
            {
              if (!regalloc_dry_run)
                {
                  ...
                  _G.stack.pushed += 2;
                   ...
                   AOP (IC_LEFT (ic))->aopu.aop_reg[0]->rIdx != D_IDX && AOP (IC_LEFT (ic))->aopu.aop_reg[0]->rIdx != E_IDX
                   && AOP (IC_LEFT (ic))->aopu.aop_reg[1]->rIdx != D_IDX && AOP (IC_LEFT (ic))->aopu.aop_reg[1]->rIdx != E_IDX)
            {
              if (!regalloc_dry_run)
                {
  ...
  if (size == 2)
    {
      ...
      if (AOP (IC_RESULT (ic))->type == AOP_HL || AOP_TYPE (IC_RESULT (ic)) != AOP_REG && AOP_TYPE (IC_RESULT (ic)) != AOP_HLREG || AOP_TYPE (IC_LEFT (ic)) != AOP_REG && AOP_TYPE (IC_LEFT (ic)) != AOP_HLREG || AOP_TYPE (IC_RESULT (ic)) == AOP_REG && AOP_TYPE (IC_LEFT (ic)) == AOP_REG && AOP (IC_RESULT (ic))->aopu.aop_reg[0] != AOP (IC_LEFT (ic))->aopu.aop_reg[1] || !regalloc_dry_run && strcmp (aopGet (AOP (IC_RESULT (ic)), 0, FALSE), aopGet (AOP (IC_LEFT (ic)), 1, FALSE)))   // Todo: More exact cost.
        {
      ...
      if (isPtrPair (AOP (left)))
        {
          if (!regalloc_dry_run)        // Todo: More exact cost.
            {
              tsprintf (buffer, sizeof (buffer), "!*pair", getPairName (AOP (left)));
