...
unsigned char op8_cost(asmop *op2)
{
  switch(op2->type)
    {
    case AOP_ACC:
...
unsigned char bit8_cost(asmop *op1)
{
  switch(op1->type)
    {
    case AOP_ACC:
        ...
        /* we need to get it byte by byte */
        ...
        else if (pairId == PAIR_IY)
          {
            if (isPair (aop))
              ...
              {
                if(!regalloc_dry_run)	// Todo: Exact cost!
                  {
                bool isUsed;
...
aopPut3 (asmop *op1, int offset1, asmop *op2, int offset2)
{
  ...
  regalloc_dry_run_cost += ld_cost(op1, op2);
}
      ...
      while (size--)
        {
          ...
          regalloc_dry_run_cost += ld_cost (AOP (result), ASMOP_A);	// Todo: More exact cost.
          offset++;
        }
